...
def course_list(request):
    ...
    if users.types == "student":
        qs = Takes.objects.filter(
            student_id=request.user.username, year=current_year, semester=current_sem
        )
        if len(qs) <= 0:
            qs = None
        ...
        qs = Teaches.objects.filter(
            teacher_id=request.user.username, year=current_year, semester=current_sem
        )
        if len(qs) <= 0:
            qs = None
        temp_name = "courses/course_list.html"
